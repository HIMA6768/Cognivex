...
R4 adds task-specific, leak-safe preprocessing on top of the canonical R2/R3 METABRIC foundation. The application includes:
...
- fresh sklearn-compatible preprocessors for clinical survival, clinical-plus-mRNA survival, and subtype classification; and
...
The current canonical scan is `DATA_QUALITY_READY_WITH_WARNINGS`. R4 uses training-only imputation/encoding/scaling, task-specific eligibility, exact original-value missing indicators, and fail-loud leakage guards without changing canonical data. R4 does **not** add survival or subtype models, predictions, metrics, gene ranking, or patient-specific output.
...
See [architecture](docs/architecture.md), [contracts](docs/api_contracts.md), [data handoff](docs/data.md), [data quality](docs/data_quality.md), [preprocessing](docs/preprocessing.md), [active provenance resolution](docs/data_provenance_resolution.md), [setup](docs/setup.md), [testing](docs/testing.md), [limitations](docs/limitations.md), and the [problem-statement migration note](docs/migration.md).
